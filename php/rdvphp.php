...
try {
  ...
  // echo $Idkine;
	...
	$res = $id->query("SELECT Date, Heure, Nom, Prenom FROM `rdv`, patient WHERE `id_kine`='".$Idkine."' AND rdv.id_compte=patient.Id"); //grâce à la requête on va chercher les infos dans la base de données
  // echo "SELECT Date, Heure, Nom, Prenom FROM `rdv`, patient WHERE `id_kine`='".$Idkine."' AND rdv.id_compte=patient.Id";
